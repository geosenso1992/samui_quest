...
class AnimalMarketScreen extends StatelessWidget {
  const AnimalMarketScreen({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    final items = game.unlockedAnimals.toList()..sort();
    final sellable =
        items.where((id) => game.getCollectedSpawnCount(id) > 0).toList();
                ...
                child: sellable.isEmpty
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final count = game.getCollectedSpawnCount(id);
                          final meta = kAnimalMetadataByName[id];
                          final rarity = meta?.rarity ?? AnimalRarity.common;
                          final border = getRarityBorderColor(rarity);
                          final coinsPer = game.getAnimalSellCoins(id);
                                ...
                                Column(
                                  children: [
                                    _SellButton(
                                      ...
                                      onTap: () {
                                        final result =
                                            game.sellAnimal(animalId: id, amount: 1);
                                        if (result.sold <= 0) return;
                                        ScaffoldMessenger.of(context).showSnackBar(
                                          SnackBar(
                                            content: Text(
                                              "Sold 1 ${_toDisplayName(id)} (+${result.coinsGained} coins)",
                                            ),
                                          ),
                                        );
                                      },
                                    ...
                                    _SellButton(
                                      ...
                                      onTap: () {
                                        ...
                                        ScaffoldMessenger.of(context).showSnackBar(
                                          SnackBar(
                                            content: Text(
                                              "Sold ${result.sold} ${_toDisplayName(id)} (+${result.coinsGained} coins)",
                                            ),
                                          ),
                                        );
                                      },
